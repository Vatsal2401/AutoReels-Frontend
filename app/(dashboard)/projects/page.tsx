...
> = {
  pending: { label: "Queued", dotClass: "bg-muted-foreground/60", textClass: "text-muted-foreground", icon: Clock },
  ...
  completed: { label: "Done", dotClass: "bg-emerald-500", textClass: "text-emerald-600", icon: CheckCircle },
    ...
    <DashboardLayout>
      <div className="h-full flex flex-col bg-background">
        <div className="w-full max-w-[1800px] mx-auto flex flex-col flex-1 min-h-0 px-3 sm:px-4 lg:px-5 pb-5">
          {/* Sticky: control bar — does not scroll */}
            ...
            <div className="flex flex-wrap items-center gap-2.5 rounded-xl border border-border/60 bg-muted/30 px-3 py-2.5 sm:gap-3 sm:px-4 sm:py-3">
              <div className="relative flex-1 min-w-[180px] max-w-[240px]">
                ...
                <Input
                  ...
                  className="h-9 pl-9 pr-3 text-sm bg-background border-border/80 focus-visible:ring-2 focus-visible:ring-primary/20"
                />
              </div>
              ...
              >
                ...
                <SelectContent align="start" className="rounded-lg border-border/80 bg-popover text-popover-foreground shadow-md min-w-[var(--radix-select-trigger-width)]">
                  <SelectItem value="all" className="rounded-md py-2 pl-8 pr-3 text-sm cursor-pointer focus:bg-muted/80">
                    All tools
                  </SelectItem>
                  {toolTypes.map((t) => (
                    <SelectItem key={t} value={t} className="rounded-md py-2 pl-8 pr-3 text-sm cursor-pointer focus:bg-muted/80">
              ...
              >
                ...
                <SelectContent align="start" className="rounded-lg border-border/80 bg-popover text-popover-foreground shadow-md min-w-[var(--radix-select-trigger-width)]">
                  ...
                  {(["pending", "processing", "rendering", "completed", "failed"] as const).map((s) => (
                    <SelectItem key={s} value={s} className="rounded-md py-2 pl-8 pr-3 text-sm cursor-pointer focus:bg-muted/80">
              ...
              >
                ...
                <SelectContent align="start" className="rounded-lg border-border/80 bg-popover text-popover-foreground shadow-md min-w-[var(--radix-select-trigger-width)]">
                  <SelectItem value="created_at-desc" className="rounded-md py-2 pl-8 pr-3 text-sm cursor-pointer focus:bg-muted/80">Newest first</SelectItem>
                  <SelectItem value="created_at-asc" className="rounded-md py-2 pl-8 pr-3 text-sm cursor-pointer focus:bg-muted/80">Oldest first</SelectItem>
                  <SelectItem value="status-asc" className="rounded-md py-2 pl-8 pr-3 text-sm cursor-pointer focus:bg-muted/80">Status A–Z</SelectItem>
                  <SelectItem value="tool_type-asc" className="rounded-md py-2 pl-8 pr-3 text-sm cursor-pointer focus:bg-muted/80">Tool A–Z</SelectItem>
                  <SelectItem value="duration-desc" className="rounded-md py-2 pl-8 pr-3 text-sm cursor-pointer focus:bg-muted/80">Duration (high)</SelectItem>
                  <SelectItem value="credit_cost-desc" className="rounded-md py-2 pl-8 pr-3 text-sm cursor-pointer focus:bg-muted/80">Credits (high)</SelectItem>
                </SelectContent>
              </Select>
              {selectedIds.size > 0 && (
                <Button
          ...
          {projectsLoading ? (
            ...
            <div className="flex-1 flex flex-col items-center justify-center py-16 text-center border border-dashed border-border/70 rounded-xl bg-muted/20">
              ...
              {allProjects.length === 0 ? (
                <Link href="/studio">
                  <Button variant="outline" size="sm" className="rounded-lg">Open Studio</Button>
                </Link>
              ) : (
                <Button variant="outline" size="sm" className="rounded-lg" onClick={() => { setSearch(""); setToolFilter("all"); setStatusFilter("all"); setPage(1); }}>
                  Clear filters
                </Button>
              )}
            </div>
            ...
            <>
              {/* Only this area scrolls — table header stays sticky inside it */}
                  ...
                  <table className="w-full text-sm table-fixed">
                    ...
                    <thead className="sticky top-0 z-10 bg-muted/80 border-b border-border/70">
                      <tr>
                        ...
                        <th className="py-3 px-3 text-left font-semibold text-foreground">Project</th>
                        <th className="py-3 px-3 text-left font-semibold text-foreground">Tool</th>
                        <th className="py-3 px-3 text-left font-semibold text-foreground">Status</th>
                        <th className="py-3 px-3 text-right font-semibold text-foreground w-24">Duration</th>
                        <th className="py-3 px-3 text-right font-semibold text-foreground w-24">Credits</th>
                        <th className="py-3 px-3 text-right font-semibold text-foreground w-28">Created</th>
              ...
              {/* Pagination — fixed at bottom, does not scroll */}
              <div className="shrink-0 mt-4 py-3 border-t border-border/70 bg-background">
                <div className="flex flex-wrap items-center justify-between gap-4">
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    <span className="text-sm text-muted-foreground">out of {filteredAndSorted.length}</span>
                  </div>
                  <div className="flex items-center gap-3">
                    ...
                    <Input
                      ...
                      className="w-12 h-8 px-2 text-center text-sm"
                    />
...
function ProjectRow({
  ...
  return (
    <tr
      ...
      onClick={(e) => {
        if ((e.target as HTMLElement).closest('input[type="checkbox"]') || (e.target as HTMLElement).closest('[data-dropdown-menu]')) return;
        onSelectRow();
      }}
    >
      ...
      <td className="py-2.5 px-3 min-w-0">
        <div className="flex items-center gap-2 min-w-0 overflow-hidden">
          ...
          <span className="font-medium text-foreground min-w-0 overflow-x-auto overflow-y-hidden whitespace-nowrap block custom-scrollbar" title={name}>
            {name}
          </span>
        </div>
      </td>
      ...
      <td className="py-2.5 pr-4 pl-2" onClick={(e) => e.stopPropagation()}>
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end" className="min-w-[120px]">
            ...
            <DropdownMenuSeparator />
            <DropdownMenuItem>
              <a href={detailHref} target="_blank" rel="noopener noreferrer" className="block w-full">Open in new tab</a>
            </DropdownMenuItem>
          </DropdownMenuContent>
        </DropdownMenu>
      </td>
